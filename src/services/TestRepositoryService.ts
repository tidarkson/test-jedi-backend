...
import {
  ...
  BulkOperationInput,
  BulkOperationResult,
  ExportedRepositoryCase,
  ExportedRepositorySuite,
  ImportRepositoryInput,
  ImportRepositoryResult,
  RepositoryExportPayload,
...
export class TestRepositoryService {
  ...
  async exportRepository(
    projectId: string,
    suiteId?: string,
  ): Promise<RepositoryExportPayload> {
    try {
      const project = await this.prisma.project.findUnique({
        where: { id: projectId },
      });
      ...
      const childrenByParent = new Map<string | null, Suite[]>();
      ...
      let rootSuiteIds: string[];

      if (suiteId) {
        const requestedSuite = suiteMap.get(suiteId);
        if (!requestedSuite) {
          throw new AppError(404, ErrorCodes.NOT_FOUND, 'Suite not found');
        }
      ...
      const includedSuiteIds = new Set<string>();
      const collectSuiteIds = (currentSuiteId: string) => {
      ...
      const casesBySuite = new Map<string, ExportedRepositoryCase[]>();
      for (const testCase of cases) {
        ...
        suiteCases.push({
          ...
          estimatedTime: testCase.estimatedTime,
          tags: (testCase.tags as string[] | null) ?? [],
      ...
      const buildExportSuite = (currentSuiteId: string): ExportedRepositorySuite => {
      ...
      if (error instanceof AppError) throw error;
      logger.error(`Error exporting repository: ${error}`);
      throw new AppError(
        500,
        ErrorCodes.INTERNAL_SERVER_ERROR,
  ...
  async importRepository(
    ...
    try {
      ...
      if (input.parentSuiteId) {
        ...
        if (!parentSuite) {
          throw new AppError(404, ErrorCodes.NOT_FOUND, 'Parent suite not found');
        }

        if (parentSuite.isLocked) {
          throw new AppError(423, 'LOCKED_RESOURCE', 'Parent suite is locked');
        }
      }

      const result = await this.prisma.$transaction(async (tx) => {
        const counts: ImportRepositoryResult = {
          suitesCreated: 0,
          casesCreated: 0,
        };
      ...
      if (error instanceof AppError) throw error;
      logger.error(`Error importing repository: ${error}`);
      throw new AppError(
        500,
        ErrorCodes.INTERNAL_SERVER_ERROR,
  ...
  private async importSuiteTree(
    tx: Prisma.TransactionClient,
    projectId: string,
    userId: string,
    parentSuiteId: string | null,
    suite: ExportedRepositorySuite,
