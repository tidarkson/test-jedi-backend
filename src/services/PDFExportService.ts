...
export class PDFExportService {
  ...
  async exportTestCases(
    ...
    try {
      ...
      logger.error('Error generating test cases PDF:', error);
      throw error;
    }
  }
  ...
  async exportTestRunResults(
    ...
    try {
      ...
      logger.error('Error generating test run PDF:', error);
      throw error;
    }
  }
   ...
   */
  private buildChartsHTML(_charts: PDFChart[]): string {
   ...
   */
  private buildHeaderHTML(branding: BrandingConfig): string {
    return `
      <div class="header">
        ${
   ...
   * Build page footer
   */
  private buildFooterHTML(branding: BrandingConfig): string {
    return `
      <div class="footer">
   ...
   */
  private getBaseStyles(branding: BrandingConfig): string {
   ...
   */
  private getPDFOptions(branding: BrandingConfig): PDFOptions {
    return {
      format: 'A4',
