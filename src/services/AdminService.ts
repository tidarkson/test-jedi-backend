...
export class AdminService {
   ...
   */
  async listOrganizationUsers(organizationId: string, page = 1, limit = 50) {
  ...
  async inviteUser(
    ...
    if (!inviterMembership || (inviterMembership.role !== 'ADMIN' && inviterMembership.role !== 'OWNER')) {
      throw new AppError(
        ...
        'Only ADMIN or OWNER can invite users',
      );
    }
    ...
    if (existingUser) {
      ...
      if (isMember) {
        throw new AppError(409, ErrorCodes.USER_ALREADY_EXISTS, 'User already member of organization');
      }
    }
    ...
    if (existingInvitation && existingInvitation.status === 'PENDING') {
      throw new AppError(409, ErrorCodes.USER_ALREADY_EXISTS, 'Invitation already pending for this email');
    }
    ...
    const inviteLink = `${config.FRONTEND_URL}/auth/accept-invitation?token=${token}`;
    ...
    logger.info(`Invitation created for ${email} to org ${organizationId}. Email sent: ${emailSent}`);
  ...
  async acceptInvitation(token: string, userId: string) {
    ...
    if (!invitation) {
      throw new AppError(404, ErrorCodes.NOT_FOUND, 'Invitation not found');
    }

    // Check if expired
    if (isPast(invitation.expiresAt)) {
      throw new AppError(410, ErrorCodes.EXPIRED, 'Invitation has expired');
    }

    if (invitation.status !== 'PENDING') {
      throw new AppError(400, ErrorCodes.INVALID_REQUEST, `Invitation already ${invitation.status.toLowerCase()}`);
    }
    ...
    if (!user || user.email !== invitation.email) {
      throw new AppError(400, ErrorCodes.INVALID_REQUEST, 'Email does not match invitation');
    }
    ...
    logger.info(`User ${userId} accepted invitation to org ${invitation.organizationId}`);
  ...
  async updateUserRole(
    ...
    if (!membership) {
      throw new AppError(404, ErrorCodes.NOT_FOUND, 'User not found in organization');
    }
    ...
    logger.info(`User ${userId} role updated from ${oldRole} to ${newRole} in org ${organizationId}`);
  ...
  async deactivateUser(
    ...
    if (!requestingMembership || (requestingMembership.role !== 'ADMIN' && requestingMembership.role !== 'OWNER')) {
      throw new AppError(
        403,
        ErrorCodes.FORBIDDEN,
        'Only ADMIN or OWNER can deactivate users',
      );
    }

    // Can't deactivate yourself
    if (userId === requestingUserId) {
      throw new AppError(400, ErrorCodes.INVALID_REQUEST, 'Cannot deactivate your own account');
    }
    ...
    logger.info(`User ${userId} deactivated from org ${organizationId}`);
   ...
   */
  async getUserActivityLog(organizationId: string, userId: string, page = 1, limit = 50) {
    const skip = (page - 1) * limit;
  ...
  async archiveProject(
    ...
    logger.info(`Project ${projectId} archived`);
   ...
   * Get project members
   */
  async getProjectMembers(organizationId: string, projectId: string, page = 1, limit = 50) {
    const skip = (page - 1) * limit;
  ...
  async addProjectMember(
    ...
    if (!orgMember) {
      throw new AppError(400, ErrorCodes.INVALID_REQUEST, 'User is not a member of this organization');
    }
    ...
    if (existing) {
      throw new AppError(409, ErrorCodes.USER_ALREADY_EXISTS, 'User already member of project');
    }
  ...
  async createCustomField(
    ...
    if (!member || (member.role !== 'ADMIN' && member.role !== 'OWNER')) {
      throw new AppError(
        403,
        ErrorCodes.FORBIDDEN,
        'Only ADMIN or OWNER can create custom fields',
      );
    }
    ...
    if (existing) {
      throw new AppError(409, ErrorCodes.USER_ALREADY_EXISTS, 'Custom field with this name already exists');
    }
  ...
  async updateCustomField(
    ...
    if (!member || (member.role !== 'ADMIN' && member.role !== 'OWNER')) {
      throw new AppError(
        403,
        ErrorCodes.FORBIDDEN,
        'Only ADMIN or OWNER can update custom fields',
      );
    }
    ...
    if (!field || field.organizationId !== organizationId) {
      throw new AppError(404, ErrorCodes.NOT_FOUND, 'Custom field not found');
    }
  ...
  async deleteCustomField(
    ...
    if (!member || (member.role !== 'ADMIN' && member.role !== 'OWNER')) {
      throw new AppError(
        403,
        ErrorCodes.FORBIDDEN,
        'Only ADMIN or OWNER can delete custom fields',
      );
    }
    ...
    logger.info(`Custom field ${fieldId} deleted`);
  ...
  async setRetentionPolicy(
    ...
    data: {
      name: string;
      description?: string;
      entityType: string; // TestRun, TestCase, Defect
      actionType: string; // ARCHIVE, DELETE
    ...
    if (!member || (member.role !== 'ADMIN' && member.role !== 'OWNER')) {
      throw new AppError(
        403,
        ErrorCodes.FORBIDDEN,
        'Only ADMIN or OWNER can set retention policies',
      );
    }
