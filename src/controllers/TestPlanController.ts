...
import {
  ...
  updatePlanSchema,
  addRunToPlanSchema,
} from '../validators/testPlan.validator';
...
export class TestPlanController {
  ...
  async listPlans(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      logger.error(`Error in listPlans: ${error}`);
      res.status(500).json({
        status: 'error',
        code: 500,
        error: ErrorCodes.INTERNAL_SERVER_ERROR,
   ...
   */
  async getPlanDetail(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      logger.error(`Error in getPlanDetail: ${error}`);
      res.status(500).json({
        status: 'error',
        code: 500,
        error: ErrorCodes.INTERNAL_SERVER_ERROR,
   ...
   * Add run to plan
   */
  async addRunToPlan(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;

      const { error, value } = addRunToPlanSchema.validate(req.body);
      ...
      logger.error(`Error in addRunToPlan: ${error}`);
      res.status(500).json({
        status: 'error',
        code: 500,
        error: ErrorCodes.INTERNAL_SERVER_ERROR,
   ...
   */
  async removeRunFromPlan(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      logger.error(`Error in removeRunFromPlan: ${error}`);
      res.status(500).json({
        status: 'error',
        code: 500,
        error: ErrorCodes.INTERNAL_SERVER_ERROR,
  ...
  async approvePlan(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      logger.error(`Error in approvePlan: ${error}`);
      res.status(500).json({
        status: 'error',
        code: 500,
        error: ErrorCodes.INTERNAL_SERVER_ERROR,
   ...
   */
  async getReleaseReadiness(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      logger.error(`Error in getReleaseReadiness: ${error}`);
      res.status(500).json({
        status: 'error',
        code: 500,
        error: ErrorCodes.INTERNAL_SERVER_ERROR,
   ...
   */
  async setBaseline(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      logger.error(`Error in setBaseline: ${error}`);
      res.status(500).json({
        status: 'error',
        code: 500,
        error: ErrorCodes.INTERNAL_SERVER_ERROR,
   ...
   */
  async getBaselineComparison(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      logger.error(`Error in getBaselineComparison: ${error}`);
      res.status(500).json({
        status: 'error',
        code: 500,
        error: ErrorCodes.INTERNAL_SERVER_ERROR,
