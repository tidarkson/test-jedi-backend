...
class ExcelExportService {
     ...
     */
    async exportTestCases(cases, _filters) {
        try {
            ...
            logger_1.logger.error('Error generating Excel export:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async exportTestRunResults(run, _cases, results) {
        try {
            const workbook = new exceljs_1.default.Workbook();
            // Summary worksheet
            ...
            logger_1.logger.error('Error generating test run Excel export:', error);
            throw error;
        }
    }
    /**
     ...
     */
    addSummarySheet(workbook, cases) {
     ...
     */
    addRunSummarySheet(workbook, run, results) {
        const worksheet = workbook.addWorksheet('Summary', { views: [{ state: 'frozen', ySplit: 1 }] });
        worksheet.columns = [
            { header: 'Metric', key: 'metric', width: 25 },
            { header: 'Value', key: 'value', width: 15 },
     ...
     */
    addRunCasesSheet(workbook, results) {
        const worksheet = workbook.addWorksheet('Cases', { views: [{ state: 'frozen', ySplit: 1 }] });
        worksheet.columns = [
            { header: 'Case ID', key: 'caseId', width: 12 },
     ...
     */
    addStepsResultsSheet(workbook, results) {
...
exports.default = new ExcelExportService();
