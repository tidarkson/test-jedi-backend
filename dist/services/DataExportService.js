...
class DataExportService {
     ...
     */
    async exportCasesAsCSV(cases, _filters) {
        try {
            const headers = [
                'ID',
                'Title',
            ...
            logger_1.logger.error('Error generating CSV export:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async exportRunResultsAsCSV(results) {
        try {
            const headers = [
                'Case ID',
                'Case Title',
            ...
            logger_1.logger.error('Error generating run results CSV:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async exportAnalyticsAsCSV(analyticsData) {
        try {
            const headers = [
                'Date',
                'Pass Rate (%)',
            ...
            const rows = (analyticsData.trendData || []).map((trend) => [
            ...
            logger_1.logger.error('Error generating analytics CSV:', error);
            throw error;
        }
    }
    /**
    ...
    async exportCasesAsJSON(cases) {
        try {
            ...
            logger_1.logger.error('Error generating JSON export:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async exportRunResultsAsJSON(run, results) {
        try {
            const data = {
                runMetadata: {
                    id: run.id,
            ...
            logger_1.logger.error('Error generating run results JSON:', error);
            throw error;
        }
    }
    /**
    ...
    async exportCasesAsXML(cases) {
        try {
            ...
            logger_1.logger.error('Error generating XML export:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async exportRunResultsAsXML(run, results) {
        try {
            const xml = this.buildXML('testrun', {
                ...
                results: {
                    result: results.map((result) => ({
                        ...
                        comment: result.comment || '',
                        duration: result.duration || '',
                        stepResults: result.stepResults && result.stepResults.length > 0
                            ? {
                                step: result.stepResults.map((sr) => ({
                            ...
                            : '',
                        defects: result.defects && result.defects.length > 0
                            ? {
                                defect: result.defects.map((defect) => ({
                                    title: defect.title,
            ...
            logger_1.logger.error('Error generating run results XML:', error);
            throw error;
        }
    }
    /**
...
exports.default = new DataExportService();
